<?php
$target_dir = "Content/fotos/";
$target_file = $target_dir . basename($_FILES["fileToUpload_avatar"]["name"]);
$uploadOk = 1;
$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));

// Zkontroluje, zda se jedná o skutečný obrázek
if(isset($_POST["submit"])) {
  $check = getimagesize($_FILES["fileToUpload_avatar"]["tmp_name"]);
  if($check !== false) {
...
if ($_FILES["fileToUpload_avatar"]["size"] > 500000) {
...
if ($uploadOk == 0) {
  ...
  if (move_uploaded_file($_FILES["fileToUpload_avatar"]["tmp_name"], $target_file)) {
    $_SESSION["foto"] = $_FILES["fileToUpload_avatar"]["name"];
    $_SESSION["ok1"] = 1;
  } else {
  $_SESSION["ok1"] = 0; 
}
